"""
Reviews API: this reads what's inside the 'reviews' table in store_copy.db and then 
returns aggregated rating info for products
"""

import sqlite3
import os

DB_PATH = os.path.join(os.path.dirname(__file__), "store_copy.db")

def get_product_rating(product_id: int) -> int:
    # Return avg rating & review count for one product
